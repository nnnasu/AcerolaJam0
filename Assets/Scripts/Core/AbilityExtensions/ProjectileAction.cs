...
public class ProjectileAction : AbilityAction {
    ...
    public override void Execute(AbilityManager manager, Vector3 target, float damage, GameplayEffect effect) {
        ...
        if (useCurve) {
            obj.Activate(curve, duration, direction);
        } else obj.Activate(speed, duration, direction);
